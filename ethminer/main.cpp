...
#endif
#if ETH_ETHASHXCL
#include <libethash-xcl/XCLMiner.h>
#endif
#if ETH_ETHASHCUDA
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
...
#if ETH_ETHASHCL || ETH_ETHASHCUDA || ETH_ETHASH_CPU || ETH_ETHASHXCL
        ...
        app.add_option("--xilinx-device,--xilinx-devices,--cxl-devices", m_CLSettings.devices, "");

        app.add_option("--xcl-binary", m_CLSettings.xilinxKernel, "", "kernel.xclbin");
        ...
        bool xcl_miner = false;
        app.add_flag("-X,--xilinx", xcl_miner, "");
    ...
    void execute()
    {
...
#endif
#if ETH_ETHASHXCL
        if (m_minerType == MinerType::XCL || m_minerType == MinerType::Mixed) {
            XCLMiner::enumDevices(m_DevicesCollection);
        }
#endif
#if ETH_ETHASHCUDA
...
#if ETH_ETHASHXCL
        if (m_CLSettings.devices.size() &&
            (m_minerType == MinerType::XCL || m_minerType == MinerType::Mixed))
        {
            for (auto index : m_CLSettings.devices) {
                if (index < m_DevicesCollection.size()) {
                    auto it = m_DevicesCollection.begin();
                    ...
                    if (!it->second.clDetected) {
                        throw std::runtime_error("Can't OpenCL subscribe a non-OpenCL FPGA device.");
                    }
                }
            }
        }
            ...
            (m_minerType == MinerType::XCL || m_minerType == MinerType::Mixed))
        {
            for (auto it = m_DevicesCollection.begin(); it != m_DevicesCollection.end(); it++)
            {
                ...
                it->second.subscriptionType = DeviceSubscriptionTypeEnum::Xilinx;
            }
        }
#endif
#if ETH_ETHASHCPU
...
#endif
#if ETH_ETHASHXCL
             << "    -X,--xilinx         Mine/Benchmark using Xilinx only" << endl
#endif
#if ETH_ETHASHCUDA
...
#endif
#if ETH_ETHASHCL
             << "                        'xcl'   Extended Xilinx options" << endl
#endif
#if ETH_ETHASHCUDA
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "xcl")
        {
            cout << "Xilinx OpenCL Extended Options :" << endl
                 << endl
                 << "    Use this extended OpenCL arguments to fine tune the performance." << endl
                 << "    Be advised default values are best generic findings by developers" << endl
                 << endl
                 << "    --xcl-devices        UINT {} Default not set" << endl
                 << "                        Space separated list of device indexes to use" << endl
                 << "                        eg --xcl-devices 0 2 3" << endl
                 << "                        If not set all available CL devices will be used"
                 << endl;
        }
